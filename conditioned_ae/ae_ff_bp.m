...
%         c0=zeros(1,size(args.WeightDecoder{i1}.r_i,1));
    ...
    for i1=1:length(delta_c0)
        [delta_up_c0{i1},dw_k,db_k]=tanh_output_bp(delta_c0{i1},args.WeightStatic.w_c{i1},cEncoder,cC0{i1});
        adw.WeightStatic.w_c{i1}=dw_k;
        adw.WeightStatic.b_c{i1}=db_k;
    end
    delta_up=sum(delta_up(:,1:size(cDecoder,2)));
    [delta_up,dw_k,db_k]=tanh_output_bp(delta_up,args.WeightStatic.w_k,cEncoder,cDecoder);
    adw.WeightStatic.w_k=dw_k;
    adw.WeightStatic.b_k=db_k;
    for i1=1:length(delta_c0)
