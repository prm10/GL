...
for i2=1:length(input0)
    ...
    [~,~,~,~,~,~,~,reconstruct]=LSTM_step_ff1(input_x11,C2,C1,args.WeightDecoder,args.LengthDecoder);
    % predict
    input_x11=[C1,zeros(1,size(label,2))];%t=1
    [~,~,~,~,~,~,~,predict]=LSTM_step_ff1(input_x11,C3,C1,args.WeightPredict,size(label,1));
    % 计算误差
    errorR=errorR+sum(sum((input(size(input,1):-1:size(input,1)-args.LengthDecoder+1,:)-reconstruct).^2))/size(reconstruct,1)/size(reconstruct,2);
