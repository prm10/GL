function [delta_c,Weight,MomWeight]=LSTM_step_bp1(args,yout,ypredict,Weight,MomWeight,x,in2,f2,z2,c,o2,y)
    %% weight initial
    % input gates
    w_i=Weight.w_i;
    r_i=Weight.r_i;
    ...
    T=size(yout,1);
    lenO=size(yout,2);
    lenC=size(Weight{1}.w_i,1)-lenO;
    delta_x=zeros(T+1,)
    w_k=Weight{end}.w_k;
    b_k=Weight{end}.b_k;
    for t=T:-1:1
        delta_k(t,:)=(-(label-predict)+delta_x(t+1,:)).*(1-predict.^2);
        delta_up(t,:)=delta_k(t,:)*w_k';
        for lay_i=length(Weight)-1:-1:1
