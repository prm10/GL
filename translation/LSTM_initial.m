...
for i1=1:length(args.encoderLayer)-2
    ...
    % input gates
    args.WeightEncoder{i1}.w_i=0.1/M*[normrnd(0,0.1,[M,N]);zeros(1,N)];
    args.WeightEncoder{i1}.r_i=0.1/N*normrnd(0,0.1,[N,N]);
    args.WeightEncoder{i1}.p_i=zeros(1,N);
    % forget gates
    args.WeightEncoder{i1}.w_f=0.1/M*[normrnd(0,0.1,[M,N]);zeros(1,N)];
    args.WeightEncoder{i1}.r_f=0.1/N*normrnd(0,0.1,[N,N]);
    args.WeightEncoder{i1}.p_f=zeros(1,N);
    % cells
    args.WeightEncoder{i1}.w_z=0.1/M*[normrnd(0,0.1,[M,N]);zeros(1,N)];
    args.WeightEncoder{i1}.r_z=0.1/N*normrnd(0,0.1,[N,N]);
    % output gates
    args.WeightEncoder{i1}.w_o=0.1/M*[normrnd(0,0.1,[M,N]);zeros(1,N)];
    args.WeightEncoder{i1}.r_o=0.1/N*normrnd(0,0.1,[N,N]);
    args.WeightEncoder{i1}.p_o=zeros(1,N);
end
% translation
M=args.encoderLayer(end-1);
N=args.encoderLayer(end);
args.WeightEncoder{length(args.encoderLayer)-1}.w_k=0.1/M*normrnd(0,0.1,[M,N]);
...
for i1=1:length(args.decoderLayer)-2
    ...
    % input gates
    args.WeightDecoder{i1}.w_i=0.1/M*[normrnd(0,0.1,[M,N]);zeros(1,N)];
    args.WeightDecoder{i1}.r_i=0.1/N*normrnd(0,0.1,[N,N]);
    args.WeightDecoder{i1}.p_i=zeros(1,N);
    % forget gates
    args.WeightDecoder{i1}.w_f=0.1/M*[normrnd(0,0.1,[M,N]);zeros(1,N)];
    args.WeightDecoder{i1}.r_f=0.1/N*normrnd(0,0.1,[N,N]);
    args.WeightDecoder{i1}.p_f=zeros(1,N);
    % cells
    args.WeightDecoder{i1}.w_z=0.1/M*[normrnd(0,0.1,[M,N]);zeros(1,N)];
    args.WeightDecoder{i1}.r_z=0.1/N*normrnd(0,0.1,[N,N]);
    % output gates
    args.WeightDecoder{i1}.w_o=0.1/M*[normrnd(0,0.1,[M,N]);zeros(1,N)];
    args.WeightDecoder{i1}.r_o=0.1/N*normrnd(0,0.1,[N,N]);
    args.WeightDecoder{i1}.p_o=zeros(1,N);
end
% output
M=args.decoderLayer(end-1);
N=args.decoderLayer(end);
args.WeightDecoder{length(args.decoderLayer)-1}.w_k=0.1/M*normrnd(0,0.1,[M,N]);
...
for i1=1:length(args.predictLayer)-2
    ...
    % input gates
    args.WeightPredict{i1}.w_i=0.1/M*[normrnd(0,0.1,[M,N]);zeros(1,N)];
    args.WeightPredict{i1}.r_i=0.1/N*normrnd(0,0.1,[N,N]);
    args.WeightPredict{i1}.p_i=zeros(1,N);
    % forget gates
    args.WeightPredict{i1}.w_f=0.1/M*[normrnd(0,0.1,[M,N]);zeros(1,N)];
    args.WeightPredict{i1}.r_f=0.1/N*normrnd(0,0.1,[N,N]);
    args.WeightPredict{i1}.p_f=zeros(1,N);
    % cells
    args.WeightPredict{i1}.w_z=0.1/M*[normrnd(0,0.1,[M,N]);zeros(1,N)];
    args.WeightPredict{i1}.r_z=0.1/N*normrnd(0,0.1,[N,N]);
    % output gates
    args.WeightPredict{i1}.w_o=0.1/M*[normrnd(0,0.1,[M,N]);zeros(1,N)];
    args.WeightPredict{i1}.r_o=0.1/N*normrnd(0,0.1,[N,N]);
    args.WeightPredict{i1}.p_o=zeros(1,N);
end
% output
M=args.predictLayer(end-1);
N=args.predictLayer(end);
args.WeightPredict{length(args.predictLayer)-1}.w_k=0.1/M*normrnd(0,0.1,[M,N]);
